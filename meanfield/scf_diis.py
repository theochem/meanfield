...
class DIISSCFSolver:
    ...
    def __call__(self, ham, overlap, occ_model, *dms):
        ...
        self._focks = [np.zeros(overlap.shape) for i in range(ham.ndm)]
        self._orbs = [Orbitals(overlap.shape[0]) for i in range(ham.ndm)]
        ...
        while self.maxiter is None or counter < self.maxiter:
            ...
            if energy_approx is not None:
                energy_change = energy_approx - min(state.energy for state in self._history.stack if
                                                    state.energy is not None)
            else:
                energy_change = None
...
class DIISState:
    ...
    def __init__(self, ndm, overlap):
        ...
        self.energy = np.nan
        self.normsq = np.nan
        self.dms = [np.zeros(overlap.shape) for i in range(self.ndm)]
        self.focks = [np.zeros(overlap.shape) for i in range(self.ndm)]
        self.commutators = [np.zeros(overlap.shape) for i in range(self.ndm)]
...
class DIISHistory:
    ...
    def __init__(self, nvector, ndm, deriv_scale, overlap, dots_matrices):
        ...
        """
        self.stack = [DIISState(ndm, overlap) for i in range(nvector)]
        self.ndm = ndm
    ...
    def log(self, coeffs):
        e_states = [state.energy for state in self.stack[:self.nused] if state.energy is not None]
        eref = min(e_states) if e_states else None
        if eref is None:
    ...
    def _build_combinations(self, coeffs, dms_output, focks_output):
        ...
        if focks_output is not None:
            ...
            for i in range(self.ndm):
                focks_stack = [self.stack[j].focks[i] for j in range(self.nused)]
